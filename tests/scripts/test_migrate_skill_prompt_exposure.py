...
def test_live_profile_requires_two_explicit_flags(monkeypatch, tmp_path):
    ...
    policy = tmp_path / "policy.yaml"
    _policy(policy)
    monkeypatch.setattr(MOD, "LIVE_PROFILE_ROOT", live_root)
...
def test_checked_in_item14_policy_counts():
    ...
    assert MOD._counts(policy) == {
        "hidden": 11,
        "names_only": 56,
        "descriptions": 11,
        "conditional": 1,
    }
    assert set(policy["hidden"]).isdisjoint(policy["names_only"])
    assert set(policy["hidden"]).isdisjoint(policy["descriptions"])
    assert set(policy["names_only"]).isdisjoint(policy["descriptions"])
