...
| `post_tool_context` | Additive context | After per-result and aggregate output budgeting; every non-empty `{"context": "..."}` return appends in registration order and is persisted before the next same-turn model call. | `tool_name`, `args`, `result`, `task_id`, `session_id`, `tool_call_id`, `turn_id`, `api_call_count`, `cwd` | Exposes finalized model-bound tool output and arguments. |
