...
| `post_tool_context` | 附加上下文 | 每个结果和整批输出预算完成后；所有非空 `{"context": "..."}` 按注册顺序追加，并在同一轮下一次模型调用前持久化。 | `tool_name`, `args`, `result`, `task_id`, `session_id`, `tool_call_id`, `turn_id`, `api_call_count`, `cwd` | 暴露最终 model-bound 工具输出和参数。 |
